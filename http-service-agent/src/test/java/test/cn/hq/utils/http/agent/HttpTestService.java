...
public interface HttpTestService {
	 ...
	 * @param id
	 * @param permited
	 * @param male 性别；作为请求参数，设置了自定义的转换器将布尔值转换为 0/1 ；
	 * @param value 值；定义为非必须的请求参数，如果值为 -1 时被忽略；
	...
	@HttpAction(method = HttpMethod.GET)
	public String content(
			@RequestParamMap ContentRequestSetting setting);
	 ...
	 * @param male
	 * @param value
	 * @param otherArg 定义一个额外的方法参数，但该参数的内容不会包含在 http 请求中；
	 * @return
	 */
	@HttpAction(path = "/content", method = HttpMethod.GET)
	public DataResponse getContent(
			@RequestParam(name = "id") String id,
			@RequestParam(name = "permited") boolean permited,
			@RequestParam(name = "male", converter=CustomBooleanConverter.class) boolean male,
			@RequestParam(name = "value", required=false, ignoreValue="-1") int value,
	...
	public String fetchContentWithError(
			@RequestParam(name = "id") String id,
			@RequestParam(name = "permited") boolean permited,
			@RequestParam(name = "male", converter=CustomBooleanConverter.class) boolean male,
			@RequestParam(name = "value", required=false, ignoreValue="-1") int value)  
	...
	public String updateAndGetContent(
	...
	public String contentBody(
			...
			@RequestBody(converter = JsonBodyConverter.class ) RequestContent requestContent);
